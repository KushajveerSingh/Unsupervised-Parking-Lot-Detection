import sys
sys.path.insert(0, 'training/m2det/')
import os
import cv2
import numpy as np
import time
...
def draw_detection(im, bboxes, scores, cls_inds, fps, thr, colors, labels):
    imgcv = np.copy(im)
    h, w, _ = imgcv.shape
    locs = []
...
def detect_parking_spaces(dir, threshold=0.2, save=False, show=False, cam=-1, gpu=False,
        config='training/m2det/configs/m2det512_vgg.py', weights='training/m2det/weights/m2det512_vgg.pth'):
    ...
    cats = [_.strip().split(',')[-1] for _ in open('training/m2det/data/coco_labels.txt','r').readlines()]
    ...
    while True:
        ...
        im2show, loc = draw_detection(image, boxes, scores, cls_inds, fps, threshold, colors=colors, labels=labels)
        locs_list[fname] = loc
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', default='training/m2det/configs/m2det512_vgg.py', type=str)
    parser.add_argument('-m', '--trained_model', default='training/m2det/weights/m2det512_vgg.pth', type=str, help='Trained state_dict file path to open')
    parser.add_argument('-f', '--directory', help='the path to images')
    ...
    parser.add_argument('--gpu', action='store_true', help='If true then model run on GPU')
